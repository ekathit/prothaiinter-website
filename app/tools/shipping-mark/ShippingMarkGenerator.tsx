...
function ShippingMarkDocument({ data, qrSvg, printPage = false }: { data: ShippingMarkData; qrSvg: string; printPage?: boolean }) {
  ...
  return (
    <article className={`${styles.shippingMark} ${printPage ? styles.printPage : ""}`}>
      ...
      <footer className={styles.documentFooter}><div className={styles.qrBlock}><div className={styles.qrCode} aria-label={`QR Code for ${qrTargetUrl}`} dangerouslySetInnerHTML={{ __html: qrSvg }} /></div><div className={styles.companyInfo}><strong>PROTHAI INTER SUPPLY &amp; SOLUTION CO.,LTD</strong><span>47/341 Kaitak Building, 5th Floor, Popular Road, Banmai, Pak Kret</span><span>Nonthaburi, 11120, Thailand</span><span>Tel: +66 (0)2-125-7096 · +66 (0)62-891-9962</span><span>Email: sales@prothaiinter.com | www.prothaiinter.com</span></div><div className={styles.footerStatus}>{data.qcPassed && <Image src="/shipping-mark/qc-passed.png" alt="QC PASSED" width={1254} height={1254} unoptimized />}</div></footer>
    </article>
  );
}
